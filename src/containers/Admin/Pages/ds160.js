    ...
    const columns = [
      {
        ...
        key: 'app_id',
        render: (text, record) => (
          <a href={`https://ds160-us.com/ds-160/application-form/token=${record._id}${record.agency ? `?agency=${record.agency}` : record.family? '?' : ''}${record.family ? `&family=${record.family}` : ''}`} target="blank">
            {text}
          </a>
        ),
      },
